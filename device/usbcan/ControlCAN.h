...
typedef  struct  _VCI_CAN_OBJ{
    UINT	ID;//帧ID
    UINT	TimeStamp;//帧的时间标识
    BYTE	TimeFlag;//是否使用时间标识
    BYTE	SendType;//发送帧类型 =0时为正常发送(发送失败会自动重发) =1时为单次发送
	BYTE	RemoteFlag;//是否是远程帧
	BYTE	ExternFlag;//是否是扩展帧
    BYTE	DataLen;//数据长度(<=8) 即CAN帧Data有几个字节 约束后面Data[8]中的有效字节
    BYTE	Data[8];//预留了8个字节的空间 受DataLen约束(如DataLen=3 则Data[0],Data[1],Data[2]有效)
