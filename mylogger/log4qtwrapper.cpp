...
void Log4QTWrapper::InitRootLogger(const QString &content)
{
    ...
    std::string logConfigFilePath = Configuration::Instance()->GetFilePath(Configuration::SystemFolderPath) + "log4qt.properties";
    std::cout<< "try to use configFile of log: " << logConfigFilePath <<std::endl;

    if (QFile::exists( logConfigFilePath.c_str() )){
        Log4Qt::PropertyConfigurator::configureAndWatch( logConfigFilePath.c_str() );
    }else{
